...
function validateContactForm(body: any): { isValid: boolean; error?: string } {
  ...
  if (!name || typeof name !== 'string' || name.trim().length === 0) return { isValid: false, error: 'Name is required.' };
  if (name.length > 100) return { isValid: false, error: 'Name is too long (max 100 characters).' };
  if (!isValidEmail(email)) return { isValid: false, error: 'Please provide a valid email address.' };
  if (!message || typeof message !== 'string' || message.trim().length === 0) return { isValid: false, error: 'Message is required.' };
  if (message.length < 5) return { isValid: false, error: 'Message must be at least 5 characters long.' };
  if (message.length > 3000) return { isValid: false, error: 'Message is too long (max 3000 characters).' };
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const fieldsHtml = `
      ...
      ${createFieldRow('Phone Number', phone ? phone.trim() : 'N/A')}
      ${createFieldRow('Subject', subject ? subject.trim() : 'General Enquiry')}
      ${createFieldRow('Message Details', (message || '').trim().replace(/\n/g, '<br/>'))}
    `;
    const html = buildHtml('NEW WEBSITE ENQUIRY', 'Contact Form Submission', fieldsHtml);
    ...
    return res.status(200).json({ success: true, message: 'Your message has been sent successfully! The TAAL team will get back to you shortly.' });
  } catch (error) {
